...
class ll_ATS(unittest.TestCase):
    ...
    def test_ll(self):
        ...
        time.sleep(3)
        driver.get("http://127.0.0.1:8004/catalog/")
        time.sleep(3)

        # Click on Books
        BookList = driver.find_element_by_xpath(
            "/html/body/div/div/div[1]/ul/li[2]/a").click()
        BookInfo = driver.find_element_by_xpath(
            "/html/body/div/div/div[2]/ul/li/a").click()

        try:
            # attempt to find the 'Logout' button - if found, logged in
            elem = driver.find_element_by_xpath("/html/body/div/div/div[1]/ul/li[5]/a").click()
            assert True
